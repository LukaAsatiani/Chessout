<template>
    <div @click="getEBE ? boardClickHandler($event) : null" v-if="getMatrix && getSize !== 0" class="board" v-bind:style="{width: getSize + 'px', height: getSize + 'px'}">
    ...
    export default {
        ...
        methods: {
            ...
            boardClickHandler($event){
                this.$store.dispatch('modes/ADD_PIECE', {square:$event.target.dataset.square})
            },
            setBoard(){
                this.$store.dispatch('modes/INIT_MATRIX')
            }
        },
        mounted(){
            this.setBoard()
